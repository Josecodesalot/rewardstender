...
class MyFirebase {
  ...
  static Future getUsername(String uid) {
    ...
    return FirebaseDatabase.instance.reference().child(Const.userId)
        .child(uid)
        .once();
  }
  ...
  static Future<Map> getClerk(String uid) async {
    ...
    DataSnapshot snap = await MyRefs.db.child(MainFields.clerks)
        .child(uid)
        .once();
  ...
  static Future<Map> getGuest(String uid) async {
    ...
    DataSnapshot snapshot = await MyRefs.db.child(MainFields.guests)
        .child(uid)
        .once();
    if (snapshot != null) {
  ...
  static Future<void> updatePoints(Map placeGuest, String placeName,
      placeId) async {
    ...
    MyRefs.db.child(MainFields.placeGuests).child(placeId).child(guestId).set(
        placeGuest);
    await MyRefs.db.child(MainFields.guestPlaces).child(guestId)
        .child(placeId)
        .set(guestPlace);
  }

  static void createTicket(HistoryTicket ht, String guestId, String placeId, String clerkId)async{
    debugPrint("create tickets called");
    String key = MyRefs.db
        .push()
        .key
        .toString();
    ht.key=key;
    ...
    MyRefs.db.child(MainFields.guestTickets).child(guestId).child(date).child(key).set(ticketDirectory);
    MyRefs.db.child(MainFields.clerkTickets).child(clerkId).child(date).child(key).set(ticketDirectory);
    MyRefs.db.child(MainFields.placeTickets).child(placeId).child(date).child(key).set(ticketDirectory);
  ...
  static Future<List> getTickets(String clerkId, String date) async {
    ...
    DataSnapshot snap = await MyRefs.db.child(MainFields.clerkTickets).child(clerkId).child(date).once();
    List keys=List();
    List tickets=List();
    if(snap.value!=null){
      ...
      for(int i =0; i<keys.length; i++){
        ...
        DataSnapshot dataSnapshot = await MyRefs.db.child(MainFields.historyTickets).child(keys[i]).once();
